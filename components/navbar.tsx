...
export default function NavBar() {
    ...
    const handleScroll = () => {
      if (!ticking) {
        requestAnimationFrame(() => {
          ...
          setIsScrolled(scrollPosition > 80); // Navbar muncul setelah scroll 80px
          ticking = false;
        });
        ticking = true;
      }
  ...
  return (
    <>
      ...
      {/* Navbar popup saat scroll */}
      <nav className={`
        fixed top-0 z-50 w-full transition-all duration-700 ease-out
        ${isScrolled 
          ? 'translate-y-0 opacity-100 glass-surface shadow-xl border-b border-border/30 scale-100' 
